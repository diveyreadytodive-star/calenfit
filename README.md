...
- 데모 컨트롤에서 정책 상태(접수 중·마감·예산 소진·확인 필요)를 전환할 수 있으며, 초기화는 가상 시드 상태로 되돌립니다. 로컬 ICS 파싱·미리보기·확정 가져오기가 구현되어 있고 VCALENDAR/VEVENT, 접힌 행, 날짜·시간대 경고를 검증합니다. 외부 캘린더 동기화는 범위에 포함하지 않습니다.
